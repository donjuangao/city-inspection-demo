...
(function () {
  ...
  (window.HELP = window.HELP || {}).m2 = {
    ...
    body:
      ...
      '<p><b>不新增调度员角色:</b>调度职能落在复核主管(常规改派)与区值班长(抢占仲裁)。</p>' +
      '<p><b>抢占调度三步:</b>承接池无空闲班组时 —— ① 占用中最低优先级件让位 ② 跨班组借调 ③ 升区值班长人裁;挂起使 SLA 停表,条件解除后回队重派。</p>'
  };
  ...
  function renderAlerts(curId) {
    ...
    var note = '<div class="oc-t3" style="margin-bottom:6px">仅列复核员确认后成立的告警;未确认的对象在复核工作台的线索队列。</div>';
  ...
  function ticketCard(t, curId) {
    ...
    return '<div' + UI.cardAttrs('ticket', { cls: 'card card-tight' }) + ' id="tk-' + UI.esc(t.id) + '">' +
      '<div class="card-hd"><b class="oc-t1">' + UI.esc(t.id) + '</b>' +
      '<span class="row" style="gap:6px;align-items:center">' + sourceBadge(t.source) + UI.cardTag('ticket', t.type) + '</span></div>' +
      ...
      (photos ? '<div class="sep"></div>' + UI.evInset('<div class="ev-grid">' + photos + '</div>') : '') +
      '</div>';
  }
  ...
  // 抢占调度卡(施工图 §6 T9;设计档 §0.8.3 在途抢占;数据模型标注 WO-8871 = 被抢占对象)
  function preemptCard() {
    var t = S.find.ticket('WO-8871');
    if (!t) return '';
    var cw = t.crew ? S.find.crew(t.crew) : null;
    var suspendOpts = S.dict().reasonCodes.suspend; // ['抢占调度','待条件(审批/物料/装备)','联动处置等待']
    ...
    var body = t.suspended
      ...
      : ('<div class="sep"></div><div class="sec-title">值班长可抢占:第二件井盖紧急件承接池无空闲班组时,挂起本单释放班组</div><div class="act-panel">' + btnsHtml + '</div>');
  ...
  function renderDispatch() {
    var crews = S.get().crews;
    return '<div class="card card-tight">' +
      '<div class="oc-t3">撤回 / 改派 / 合并 / 拆单均为显名动作,理由码入动作日志。常规改派权在复核主管,抢占仲裁权在区值班长。</div>' +
      '</div>' +
