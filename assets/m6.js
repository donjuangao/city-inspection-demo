/* ===== 模块 m6 视图 · 系统管理 · 归属 W2 工兵(施工图 §7/§8 裁定②:模块拆独立文件,文件互斥) =====
   ...
   规格来源:
   - 施工图 §7 m6:角色表 / 参数变更(影子试算)/ 类目机器直派开关(超级管理员)/ 值班表
   - 设计档 §0.6:角色·账号·权限·数据权限(R85④ 角色口径 + 三非人账号;超级管理员关自动车道条款 R72)
   - R86⑥:六 tab + hash 子路由 #/m6/<tab>(roles/rules/lanes/params/duty/ontology)
   - R86 A2:规则引擎 tab —— RULES 36 条按线分组白盒展示;参数调整 → S.commit('rule_param_change') 影子试算 + 审批留痕
...
(function (w) {
  ...
  /* ================= 0 · tab 定义与子路由 ================= */
  var TABS = [
    { key: 'roles', label: '角色权限', sub: '人角色三类 + 非人三账号的权限范围' },
    { key: 'rules', label: '规则引擎', sub: '判据规则总表 · 按组折叠 · 参数可调(影子试算)· AI 复验策略白盒' },
    { key: 'lanes', label: '通道与类目开关', sub: '五类通道口径 + 类目机器直派开关(超级管理员)' },
  ...
  (w.HELP = w.HELP || {}).m6 = {
    ...
    body:
      ...
      '<p><b>通道与类目开关:</b>派单是处置调度,不是定性 —— 机器直派件的定性签字仍在人,并行复核窗内可撤回。超级管理员关闭类目机器直派 = 安全侧动作即时生效,重开需按数据判据批准。</p>' +
  ...
  var ROLE_DESC = {
    '区复核员': '工作台读写 / 告警读。可执行:确认 / 驳回 / 推翻规则判定 / 机器直派撤回 / 提级②(急修 × 多源确证 → 机器直派,原因必填,件全量拘抽审)/ 批量确认 / 存疑归档 / 立案·结案 / 转办 / 复验人裁与打回。',
    '复核主管': '复核员全部权限 + 抽审 / 撤销 / 兜底读写。另可:高危 override(红色动作,双确认)/ 证据冻结 / 调度改派·合并·拆单 / 挂起与强制回队 / 提级①(养护 / 观察→急修,原因必填;提级②→机器直派复核员即可发起)/ 台账读写 / 总览本区(模块 5)/ 发起参数变更(走审批)/ 账号与角色授予(走审批,动作留痕);夜间值班时同岗执行升级链第三级兜底、加急单作业时窗豁免批准与过载三步的借调与仲裁;机器直派关闭后的区申诉发起人之一。',
    '超级管理员': '跨区只读 + 框架标准 + 类目机器直派入池开关 + 月报。关车道 = 安全侧动作即时生效;重开需按数据判据批准。'
  };
  var SVC_DESC = {
    '规则引擎': '规则校验闸 / 自动车道(记账·升格·机械驳回·设备工单)/ 预警广播 / 机器直派自动派单 / 超时兜底派单的执行身份;机械自动化也不豁免审计,每步入动作日志。',
  ...
  function fastlaneRow(cat) {
    ...
    var toggleBtn = on
      ? { action: 'toggle_fastlane', label: '关闭机器直派', cls: 'btn-danger', params: { cat: cat, on: false, reason: '超级管理员关闭该类目机器直派(治理动作)' } }
  ...
  function fastlaneBlock() {
    ...
    return '<div class="card">' +
      ...
      '<span class="tiny">仅超级管理员身份可点(灰态见未满足校验);关 = 即时生效 + 强制通知横幅 + 区申诉入口</span></div>' +
      '<div style="overflow-x:auto"><table class="tb"><thead><tr><th>类目</th><th>当前状态</th><th>超级管理员操作</th><th>区申诉</th></tr></thead>' +
  ...
  var OBJ_TYPES = [
    ...
    { name: '线索', key: 'clues', fields: '线索号 · 业务线 · 级别 · 置信度 · 通道 · 状态 · 规则校验项' },
